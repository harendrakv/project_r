...
class TextSimilarityScore:
    ...
    def get_resumes_similarity_given_jd(self, jd_path):
        ...
        for i in range(len(self.resumes)):
            score = self.get_average_similarity(
                self.resumes[i]['cleantext'], jd['cleantext'])
    ...
    def get_jd_similarity_given_resume(self, resume_path):
        resume = json.load(open(os.path.join(
            FilesUtility.user_file_path, FilesUtility.resume_save_path, resume_path)))
        jd_scores = []
        jd_files = []
        for i in range(len(self.jds)):
            score = self.get_similarity(
                self.jds[i]['cleantext'], resume['cleantext'])
        ...
        for j, tag in enumerate(tags):
            ...
            if len(jd_keywords) > 0:
                ...
                for i in range(len(self.resumes)):
                    ...
                    matches = self.get_keywords_matches(
                ...
                matched_df = pd.DataFrame(
                    {"resumes": resume_files, tag+"_matched": matching_keywords, tag+"_score": keywords_match_score})
                dflist.append(matched_df)
            else:
                pass
        fields_df = pd.concat(dflist, axis=1, join="inner")
        fields_df = fields_df.loc[:, ~
                                  fields_df.columns.duplicated(keep='last')]
        return fields_df
