...
def create_database(phraseMatcher):
    extracted_resumes_path = os.path.join(FilesUtility.user_file_path, FilesUtility.resumes_save_path)
    resumeFiles = os.listdir(extracted_resumes_path)
    final_database=[]
    i = 0 
    while i < len(resumeFiles):
        file = resumeFiles[i]
        print("Processing: ", os.path.basename(file), "        ..."+str(i+1)+"/"+str(len(resumeFiles)))    
        try:
            text_dict = json.load(open(os.path.join(extracted_resumes_path, file)))  
            ...
            df1 = pd.DataFrame(df.Keywords_List.str.split('&').tolist(),columns = ['Subject','Keyword', 'Count'])
...
#    data[data.values < threshold] = 0
    plot_df1 = data.sort_values(by=[subject], ascending=False)[:no_samples_to_plot]
    ...
    ax = plot_df.plot.barh(title="Top "+str(no_samples_to_plot)+" Resumes sorted by "+ subject+" out of "+str(len(data)), figsize=(12,6), legend=False, stacked=True)
    labels = []
...
#            label = str(plot_df.loc[i][j])            
    ...
    plt.tight_layout()
    if outpath==None:
        plt.savefig(os.path.join(os.getcwd, 'output', 'resumes_sorted_by_{}.png'.format(subject)))
    else:
        plt.savefig(os.path.join(outpath, 'resumes_sorted_by_{}.png'.format(subject)))
    plt.close('all')
...
def process_resumes_for_keywords_matching(keyword_file,threshold_for_keyword_count_perSubject=0, outpath=None):  
    ...
    data_df1 = data_df1['Keyword'].groupby([data_df1['resumes'], data_df1['Subject']]).count().unstack()
